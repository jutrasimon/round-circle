...
const soldierProfiles=[
 {id:'guardian',name:'Gardien',role:'Un rempart lent, très résistant, au contact.',color:'#e9b96b',duration:14,stats:{maxHp:220,damage:7,speed:.8,range:1.8,cooldown:1.4}},
 {id:'sniper',name:'Veilleur',role:'Très fragile. Un tir lourd à très longue portée.',color:'#91c9ff',duration:20,stats:{maxHp:28,damage:40,speed:1.2,range:8,cooldown:4}},
 {id:'gunner',name:'Rafale',role:'Tirs très rapides, portée courte, résistance modeste.',color:'#ed94c9',duration:12,stats:{maxHp:48,damage:4,speed:2,range:3,cooldown:.18}},
 {id:'scout',name:'Éclaireur',role:'Recrutement rapide et grande mobilité.',color:'#9de2b1',duration:6,stats:{maxHp:65,damage:11,speed:5.8,range:2.4,cooldown:.65}}
];
...
class Simulation{
 constructor(cfg={}){this.cfg={...defaults,...cfg};this.profile=clone(profileDefaults);this.monsterSpecs=clone(types);this.soldierProfiles=clone(soldierProfiles);this.nextId=1;this.reset();}
 entity(kind,name,stats){if(this.metrics)this.metrics.created[kind]=(this.metrics.created[kind]||0)+1;const key=kind==='actor'?'actorHp':kind==='building'?'buildingHp':['train','wagon'].includes(kind)?'trainHp':null;stats={...stats,maxHp:stats.maxHp*this.multiplier(key)};return {id:this.nextId++,kind,name,hp:stats.maxHp,maxHp:stats.maxHp,damage:0,speed:0,range:0,cooldown:1,timer:0,x:0,z:0,angle:0,...stats};}
 reset(){this.ended=false;this.result=null;this.metrics={created:{},lost:{},damage:{},shots:{},killsByType:{},distance:0,regenerated:0,boardings:0,wagonsProduced:0,wavesStarted:0,wavesCompleted:0};this.bonuses={};this.resetId=(this.resetId||0)+1;this.cfg.trainSpeed=clampTrainSpeed(this.cfg.trainSpeed);this.events=[];this.time=0;this.angle=0;this.kills=0;this.collisions=0;this.contacts=new Set();this.train=this.entity('train','Locomotive',{maxHp:this.cfg.trainHp,damage:this.cfg.trainDamage,speed:this.cfg.trainSpeed,range:this.cfg.trainRange,cooldown:this.cfg.trainCooldown});this.wagons=[];this.actors=[];this.monsters=[];this.buildings=[];this.buildingPopElapsed=0;for(let i=0;i<Math.max(0,Math.min(12,this.cfg.buildingCount));i++)this.addBuilding(true);for(let i=0;i<3;i++)this.addWagon();this.positionTrain();this.onReset?.();}
 ...
 get(id){return this.train.id===id?this.train:this.wagons.find(e=>e.id===id)||this.actors.find(e=>e.id===id)||this.monsters.find(e=>e.id===id)||this.buildings.find(e=>e.id===id);}
 ...
 produce(home){if(!home||home.hp<=0||home.dead)return null;const p=this.population();if(this.train.hp>0&&this.cfg.capacity>0&&p.aboard>=p.seats&&this.wagons.length<16){const w=this.addWagon();if(w){this.metrics.wagonsProduced++;this.events.push({type:'wagon-born',id:w.id,homeId:home.id,x:w.x,z:w.z});}return w;}return this.spawnActor(home);}
 queueTraining(homeId,profileId){const home=this.buildings.find(b=>b.id===homeId),profile=this.soldierProfiles.find(p=>p.id===profileId);if(this.ended||!home||home.hp<=0||home.dead||home.training||!profile||!this.cfg.production)return false;if(this.population().total+this.buildings.filter(b=>b.hp>0&&b.training).length>=this.cfg.actorLimit)return false;home.training={profile:clone(profile),elapsed:0};home.readyNotice=null;this.events.push({type:'training-started',homeId,profileId});return true;}
 trainAt(home,dt){const job=home.training;if(!job||!this.cfg.production)return;job.elapsed=Math.min(job.profile.duration,job.elapsed+dt*this.multiplier('production'));if(job.elapsed<job.profile.duration)return;const a=this.spawnActor(home,{...job.profile.stats,profileId:job.profile.id,profileName:job.profile.name});if(!a)return;const pop=this.population();if(this.train.hp>0&&pop.aboard>=pop.seats&&this.wagons.length<16){const wagon=this.addWagon();if(wagon){this.metrics.wagonsProduced++;this.events.push({type:'wagon-born',id:wagon.id,homeId:home.id,x:wagon.x,z:wagon.z});}}a.name=job.profile.name+' '+a.id;home.readyNotice=job.profile.name+' prêt · lancer un autre recrutement';home.training=null;this.events.push({type:'training-complete',homeId:home.id,profileId:job.profile.id,name:job.profile.name});}
 ...
 step(dt){this.time+=dt;const previous=this.angle;this.train.speed=clampTrainSpeed(this.train.speed);const speed=this.train.hp>0?this.train.speed:0;this.metrics.distance+=speed*dt;this.angle+=speed/RAIL*dt;this.positionTrain();
 this.popBuildings(dt);
 for(const b of this.buildings){if(b.hp<=0){b.dead=true;continue;}if(b.dead)continue;const recovered=Math.min(b.maxHp-b.hp,Math.max(0,b.regen)*this.multiplier('regen')*dt);b.hp+=recovered;this.metrics.regenerated+=recovered;if(this.cfg.manualProduction){this.trainAt(b,dt);}else if(this.cfg.production&&b.spawnRate>0){b.spawnProgress=Math.min(1,b.spawnProgress+dt*b.spawnRate*this.multiplier('production')/60);if(b.spawnProgress>=1&&this.produce(b))b.spawnProgress=0;}}
 const occupied=new Map();for(const a of this.actors)if(a.hp>0&&a.wagonId)occupied.set(a.wagonId,(occupied.get(a.wagonId)||0)+1);
 ...
 let reachable=choices.filter(e=>this.distance(m,e)<=Math.max(0,m.range)+1e-6);const committed=homes.find(e=>e.id===m.siegeTargetId);
 if(m.behavior==='siege'&&homes.length){const interceptors=reachable.filter(e=>['train','wagon'].includes(e.kind)&&this.distance(m,e)<=.8);choices=homes;reachable=interceptors.length?interceptors:homes.filter(e=>this.distance(m,e)<=Math.max(0,m.range)+1e-6);}
 ...
 finishRun(extra={}){if(this.result)return this.result;this.ended=true;this.result=clone({time:this.time,radius:this.cfg.vortexRadius,metrics:this.metrics,population:this.population(),kills:this.kills,collisions:this.collisions,train:{hp:this.train.hp,maxHp:this.train.maxHp},wagons:this.wagons.length,buildings:this.buildings.filter(b=>b.hp>0&&!b.dead).length,monsters:this.monsters.filter(m=>m.hp>0).length,bonuses:this.bonuses,...extra});return this.result;}
 ramWear(speed){return speed>0?Math.max(5,speed)*this.cfg.ramSelfDamage:0;}
 ram(previous,speed){const current=new Set(),vehicles=[this.train,...this.wagons];for(let i=0;i<vehicles.length;i++){const w=vehicles[i];if(w.hp<=0)continue;const end=this.angle-i*.22,start=previous-i*.22;for(const m of this.monsters){if(m.hp<=0)continue;const radius=Math.hypot(m.x,m.z),hitRadius=.43+m.size*.34;if(Math.abs(radius-RAIL)>hitRadius)continue;const angle=Math.atan2(m.z,m.x),delta=this.angleDelta(start,angle),travel=end-start,half=Math.sqrt(Math.max(0,hitRadius**2-(radius-RAIL)**2))/RAIL+.04;const swept=delta>=-half&&delta<=travel+half;const contact=Math.abs(this.angleDelta(end,angle))<=half;const key=w.id+':'+m.id;if(contact)current.add(key);if(speed>.01&&swept&&!this.contacts.has(key)){this.damage(m,speed*this.cfg.ramDamage*this.multiplier('ram')*(1-(m.armor||0)));this.damage(w,this.ramWear(speed));this.collisions++;this.events.push({type:'ram',x:m.x,z:m.z,speed,id:w.id});if(w.hp<=0)break;}}}this.contacts=current;}
