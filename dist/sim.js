...
class Simulation{
 constructor(cfg={}){this.cfg={...defaults,...cfg};this.profile=clone(profileDefaults);this.monsterSpecs=clone(types);this.nextId=1;this.reset();}
 ...
 for(const b of this.buildings){if(b.hp<=0){b.dead=true;continue;}if(b.dead)continue;const recovered=Math.min(b.maxHp-b.hp,Math.max(0,b.regen)*this.multiplier('regen')*dt);b.hp+=recovered;this.metrics.regenerated+=recovered;if(this.cfg.production&&b.spawnRate>0){b.spawnProgress=Math.min(1,b.spawnProgress+dt*b.spawnRate*this.multiplier('production')/60);if(b.spawnProgress>=1&&this.produce(b))b.spawnProgress=0;}}
 for(const a of this.actors){if(a.hp<=0||a.wagonId)continue;const old=a.angle;if(a.radius>STREET){a.radius=Math.max(STREET,a.radius-Math.min(a.speed,1.2)*dt);}else{a.angle+=a.speed/STREET*dt;for(let i=0;i<this.wagons.length;i++){const w=this.wagons[i];if(w.hp<=0||this.passengers(w).length>=w.capacity)continue;const start=previous-(i+1)*.22;const d=this.angleDelta(start,old),relative=(this.angle-previous)-(a.angle-old);const crossed=relative>=0?d>=-.09&&d<=relative+.09:d<=.09&&d>=relative-.09;if(crossed||Math.abs(this.angleDelta(a.angle,w.angle))<.09){a.wagonId=w.id;this.metrics.boardings++;this.events.push({type:'board',id:a.id});break;}}}a.x=Math.cos(a.angle)*a.radius;a.z=Math.sin(a.angle)*a.radius;}
 this.positionTrain();
