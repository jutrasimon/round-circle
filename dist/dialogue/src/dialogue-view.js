export class DialogueView{
 constructor(host,engine,{assetBase='./',onOpen=()=>{},onClose=()=>{},lettersPerSecond=45}={}){
  this.host=host;this.engine=engine;this.options={assetBase,onOpen,onClose,lettersPerSecond};this.timer=null;this.wasOpen=false;this.lastFocus=null;
 ...
 reveal(){if(!this.state)return;clearInterval(this.timer);this.timer=null;this.text.textContent=this.state.node.text;this.actions.querySelectorAll('button').forEach(b=>b.disabled=false);}
 render(state){clearInterval(this.timer);this.state=state;if(!state){this.host.hidden=true;if(this.wasOpen){this.wasOpen=false;this.options.onClose();this.lastFocus?.focus();}return;}
  if(!this.wasOpen){this.wasOpen=true;this.lastFocus=document.activeElement;this.options.onOpen();}this.host.hidden=false;
  const n=state.node,portrait=this.host.querySelector('.rct-portrait'),url=this.options.assetBase+state.character.poses[n.pose];
 ...
 destroy(){clearInterval(this.timer);this.engine.onChange=()=>{};this.host.removeEventListener('keydown',this.keyHandler);this.host.hidden=true;if(this.wasOpen)this.options.onClose();this.lastFocus?.focus();}
}
