export class DialogueView{
 ...
 render(state){clearInterval(this.timer);clearTimeout(this.advanceTimer);this.state=state;if(!state){this.host.hidden=true;if(this.wasOpen){this.wasOpen=false;this.options.onClose();this.lastFocus?.focus();}return;}
  ...
  const n=state.node,portrait=this.host.querySelector('.rct-portrait'),url=this.options.assetBase+state.character.poses[n.pose];this.host.classList.toggle('choice',n.type==='choice');this.host.dataset.speaker=n.speaker;
