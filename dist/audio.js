...
function install(){
 ...
 const box=document.createElement('details');box.id='audioSettings';box.innerHTML='<summary>Musique et sons</summary><p>Fortress of Bone · musique en boucle. Huit effets réutilisés, avec fichier et volume au choix.</p>';
 ...
 function sound(id,preview=false){if(!unlocked||document.hidden||ctx.state!=='running'||(!preview&&(!settings.enabled||!settings.effectsEnabled||settings.effects===0||active.size>=12||!gate.allow(id,ctx.currentTime))))return;
  ...
  if(preview)previewNode=node;active.add(node);node.onended=()=>{active.delete(node);if(previewNode===node)previewNode=null;node.disconnect();gain.disconnect();};node.start();
 }
 ...
 for(const [id,c] of Object.entries(cues)){
  ...
  const button=document.createElement('button');button.type='button';button.textContent='▶';button.setAttribute('aria-label','Écouter · '+c.label);button.onclick=async()=>{button.disabled=true;try{await unlock();const selected=mix[id].file;const buffer=await loadFile(selected);if(buffer&&mix[id].file===selected)sound(id,true);}finally{button.disabled=false;}};row.append(button);editor.append(row);
 }
