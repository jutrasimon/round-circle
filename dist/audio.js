...
function install(){
 ...
 let settings={enabled:true,music:.28,effects:.45};try{const saved=JSON.parse(localStorage.getItem('round-circle-audio-v1'));if(saved){settings.enabled=saved.enabled!==false;for(const k of ['music','effects'])if(Number.isFinite(saved[k]))settings[k]=Math.max(0,Math.min(1,saved[k]));}}catch{}
 ...
 async function unlock(){try{if(!ctx){ctx=new (window.AudioContext||window.webkitAudioContext)();bus=ctx.createGain();bus.connect(ctx.destination);}await ctx.resume();unlocked=true;sync();void load();await playMusic();}catch{status.textContent='Audio indisponible dans ce navigateur.';}}
 ...
 function sound(id){if(!settings.enabled||!unlocked||document.hidden||ctx.state!=='running'||settings.effects===0||active.size>=12||!gate.allow(id,ctx.currentTime))return;let node;const gain=ctx.createGain();gain.connect(bus);gain.gain.value=.55;
 ...
 for(const [id,c] of Object.entries(cues)){const button=document.createElement('button');button.type='button';button.textContent=c.label;button.onclick=async()=>{settings.enabled=true;await unlock();await load();sound(id);};tests.append(button);}
