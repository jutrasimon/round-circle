...
function validate(source){
 ...
 return copy({version:1,id:source.id,name:source.name,stats,waves,bonuses,dialogue:{library:dialogue.library,cues,choiceScale:dialogue.choiceScale,autoAdvanceMs:dialogue.autoAdvanceMs},endings});
}
async function load(url){const response=await fetch(url,{signal:AbortSignal.timeout(10000)});if(!response.ok)throw Error('Dictionnaire de partie introuvable');const source=await response.json(),{validateLibrary}=await import('./dialogue/src/dialogue-core.js?v=027');source.dialogue.library=validateLibrary(source.dialogue.library);return validate(source);}
